...
async function generateCollectionTasksInner(client: PoolClient): Promise<void> {
  ...
  try {
    ...
    log.info(`ERP查询到 ${rawDebts.length} 条欠款记录`);
    ...
    if (hoardFilteredCount > 0) {
      log.info(`排除 ${hoardFilteredCount} 条压单欠款`);
    }
    ...
    log.info(
      `发现 ${qualifiedDebts.length} 条新增需催收的欠款，入催原因: [${entryReasons.join(', ')}]`
    );
    ...
    // 9. 为每组生成任务（事务内）
    await createTasksInTransaction(
      client,
      groups,
      existingActiveTasks,
      todayStr,
...
/** 按客户+逾期触发日期分组 */
function groupByConsumerAndOverdueDate(
  debts: EnrichedDebtRecord[]
): Map<string, EnrichedDebtRecord[]> {
  const groups = new Map<string, EnrichedDebtRecord[]>();
  for (const debt of debts) {
...
async function createTasksInTransaction(
  ...
  for (const [key, debts] of groups) {
    ...
    if (existingActiveTasks.has(key)) {
      ...
      log.info(
        `跳过重复任务: ${consumerName} 逾期日=${overdueDateStr}，已有活跃任务 #${existingActiveTasks.get(key)}`
      );
      continue;
    }
    ...
    if (managerName) {
      const userResult = await client.query(`SELECT id FROM users WHERE name = $1 LIMIT 1`, [
        managerName,
      ]);
    ...
    await client.query(
      ...
      [
        taskId,
        `系统自动生成催收任务，批次类型=${batchType}，包含${debts.length}笔欠款，入催原因=[${groupReasons.join(',')}]`,
      ]
    );
  }
  ...
  if (skippedGroups.length > 0) {
    log.info(`跳过 ${skippedGroups.length} 个重复任务组`);
  }
  log.info(`成功生成 ${taskIdMap.size} 个催收任务`);

  // 发送任务创建通知
  try {
    await sendTaskCreatedNotifications(Array.from(taskIdMap.entries()));
  } catch (notifyErr) {
    log.error('发送任务创建通知失败:', notifyErr);
  }
}
