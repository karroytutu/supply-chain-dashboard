 ...
 * 事务内查询顺序：
 *   BEGIN → SELECT FOR UPDATE → UPDATE form_data → INSERT action → [INSERT comment] → COMMIT
 */
function setupTransaction(selectResult: any, extraQueryResults: any[] = []) {
  const queryMock = jest.fn()
    .mockResolvedValueOnce({} as any)           // BEGIN
    .mockResolvedValueOnce(selectResult)          // SELECT ... FOR UPDATE
    .mockResolvedValueOnce({} as any);            // UPDATE form_data
...
describe('updateInstanceFormData', () => {
  ...
  it('合并新数据到已有 form_data', async () => {
    const mockClient = setupTransaction(
      { rows: [{ form_data: { a: 1, b: 2 }, current_node_order: 1 }] },
      [{ rows: [] }, { rows: [] }], // INSERT action, (no comment)
    );

    await updateInstanceFormData(1, 10, '张三', { b: 20, c: 30 });

    // client.query 调用顺序：BEGIN, SELECT FOR UPDATE, UPDATE form_data, INSERT action, COMMIT
    const updateCall = mockClient.query.mock.calls[2]; // 第3次调用 = UPDATE
  ...
  it('插入 action_type=update 操作记录（在事务内）', async () => {
    const mockClient = setupTransaction(
      { rows: [{ form_data: {}, current_node_order: 2 }] },
      [{ rows: [] }, { rows: [] }], // INSERT action, INSERT comment
    );

    await updateInstanceFormData(1, 10, '张三', { x: 1 }, '更新了数据');

    // INSERT action 是事务内的第4次 client.query（BEGIN=0, SELECT=1, UPDATE=2, INSERT action=3）
  ...
  it('不传 comment 时不插入 comment 记录', async () => {
    const mockClient = setupTransaction(
      { rows: [{ form_data: {}, current_node_order: 1 }] },
      [{ rows: [] }], // 仅 INSERT action，无 INSERT comment
    );

    await updateInstanceFormData(1, 10, '张三', { x: 1 });

    // 调用次数：BEGIN(1) + SELECT(1) + UPDATE(1) + INSERT action(1) + COMMIT(1) = 5
    expect(mockClient.query).toHaveBeenCalledTimes(5);
  });

  it('实例 form_data 为 null 时初始化为空对象', async () => {
    const mockClient = setupTransaction(
      { rows: [{ form_data: null, current_node_order: 1 }] },
      [{ rows: [] }],
    );
  ...
  it('无变更时 details 为 null', async () => {
    const mockClient = setupTransaction(
      { rows: [{ form_data: { a: 1 }, current_node_order: 1 }] },
      [{ rows: [] }],
    );
