 ...
 *
 * 注：generateCollectionTasks 已迁移至 OA 审批系统 (services/oa/ar-collection-creator.ts)
 */
...
import type { TaskStatus } from '../ar-collection/ar-collection.types';
import { detectAllHoardChanges } from './erp-hoard-detect';
...
/** 失效指定任务相关的所有缓存（内联自 ar-collection.repository） */
function invalidateCollectionTaskCache(taskId?: number): void {
...
export async function syncERPDebts(): Promise<void> {
  ...
  try {
    ...
    for (const debt of erpDebts) {
      ...
      if (!existing) {
        // 新记录 - 暂不插入明细(等 OA 审批通过后 ar-collection-creator 创建任务时关联)
        skippedCount++;
    ...
    log.info(
      `同步完成: 跳过(待任务生成)=${skippedCount}, 更新=${updateCount}, 消失=${removedCount}, 耗时=${duration}ms`
    );
...
export async function checkExtensionExpiry(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      for (const task of result.rows) {
        invalidateCollectionTaskCache(task.id);
      }
  ...
  try {
    ...
    try {
      ...
      for (const taskId of affectedTaskIds) {
        invalidateCollectionTaskCache(taskId);
      }
...
async function handleRemovedDebt(detail: LocalDetail): Promise<void> {
  ...
  // 关闭关联明细（避免孤儿数据被重新纳入催收指标计算）
  await appQuery(
    `UPDATE ar_collection_details SET status = 'closed' WHERE id = $1`,
    [detail.id]
  );
  ...
  invalidateCollectionTaskCache(detail.task_id);
