...
import { syncERPDebts, checkExtensionExpiry, checkHoldExpiry } from './erp-debt-sync.task';
...
const mockCacheInvalidate = cache.invalidate as jest.Mock;
...
describe('checkExtensionExpiry', () => {
  ...
  it('处理到期延期任务: 恢复催收状态 + 失效缓存', async () => {
    ...
    // 缓存失效
    expect(mockCacheInvalidate).toHaveBeenCalledWith('ar:collection:tasks:');
    expect(mockCacheInvalidate).toHaveBeenCalledWith('ar:collection:task:1');
    expect(mockCacheInvalidate).toHaveBeenCalledWith('ar:collection:task:2');
