 ...
 */
export async function sendAssessmentNotifications(records: AssessmentRecordRow[]): Promise<void> {
  ...
  for (const [userId, userRecords] of userGroupMap) {
    ...
    try {
      ...
      await sendWorkNotification([dingtalkId], notification.title, notification.markdown);
      sentCount++;
    } catch (error) {
      log.error(`发送通知失败: userId=${userId}`, error);
    }
  }

  log.info(`通知发送完成: ${sentCount}/${userGroupMap.size}`);
}
...
function buildNotificationContent(
  ...
  for (const [category, groupRecords] of categoryGroups) {
    const categoryLabel =
      ASSESSMENT_CATEGORY_LABELS[category as keyof typeof ASSESSMENT_CATEGORY_LABELS] || category;

    const tableRows = groupRecords
      .map(r => {
