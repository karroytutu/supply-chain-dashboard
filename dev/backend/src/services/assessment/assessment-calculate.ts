...
export async function runCalculation(
  ...
  if (matchingRules.length === 0) {
    log.info(`无匹配规则, context: ${JSON.stringify(ctx)}`);
  ...
  for (const rule of matchingRules) {
    try {
      log.info(`执行规则: ${rule.category}:${rule.ruleType} (${rule.name})`);
      ...
      if (results.length > 0) {
        ...
        newRecords += count;
        log.info(`规则 ${rule.ruleType}: 计算 ${results.length} 条, 写入 ${count} 条`);
      }
    } catch (error) {
      // 单条规则失败不影响其他规则执行
      log.error(`规则 ${rule.category}:${rule.ruleType} 执行失败:`, error);
    }
  }
