-- 089: 催收表单 schema v2 — 完善账单明细 + 客户授信账期
-- 变更：
--   1. billDetails 隐藏 billNo（内部编号），新增 orderNo/workTime/writeOffAmount/billNote
--   2. 只读区新增 maxDebtDays（最大欠款天数）、maxDebtOrderNum（最大欠款单数）
-- form_schema 以代码定义为准（ar-collection.ts），此处同步更新数据库
...
ON CONFLICT (code) DO UPDATE SET
  ...
  -- workflow_def 保持不变，避免覆盖生产环境手动调整
