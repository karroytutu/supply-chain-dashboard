...
export const getDashboard = async (req: Request, res: Response) => {
  try {
    ...
    log.error('获取Dashboard数据失败:', error);
    res
      .status(500)
...
export const getWarningProductsController = async (req: Request, res: Response) => {
  try {
    ...
    log.error('获取预警商品列表失败:', error);
    res
      .status(500)
      .json(
        buildErrorResponse(500, error instanceof Error ? error.message : '获取预警商品列表失败')
      );
  }
};
...
export const getCategoryTreeController = async (req: Request, res: Response) => {
  try {
    ...
    log.error('获取品类树数据失败:', error);
    res
      .status(500)
      .json(buildErrorResponse(500, error instanceof Error ? error.message : '获取品类树数据失败'));
  }
};
...
export const getCategoryOutOfStockController = async (req: Request, res: Response) => {
  try {
    ...
    log.error('获取品类缺货商品列表失败:', error);
    res
      .status(500)
      .json(
        buildErrorResponse(500, error instanceof Error ? error.message : '获取品类缺货商品列表失败')
      );
  }
};
