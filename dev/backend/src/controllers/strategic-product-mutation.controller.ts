...
export async function addStrategicProductsController(req: Request, res: Response) {
  try {
    ...
    if (!goodsIds || !Array.isArray(goodsIds) || goodsIds.length === 0) {
      return res.status(400).json(buildErrorResponse(400, '请提供商品ID列表'));
    }

    if (!userId) {
      return res.status(401).json(buildErrorResponse(401, '未登录'));
    }
...
export async function deleteStrategicProductController(req: Request, res: Response) {
  try {
    ...
    if (success) {
      res.json(buildSuccessResponse(null, '删除成功'));
    } else {
...
export async function confirmStrategicProductController(req: Request, res: Response) {
  try {
    ...
    if (!action || !['confirm', 'reject'].includes(action)) {
      return res.status(400).json(buildErrorResponse(400, '无效的操作类型'));
    }
    ...
    if (result) {
      res.json(buildSuccessResponse(result, action === 'confirm' ? '确认成功' : '已驳回'));
    } else {
