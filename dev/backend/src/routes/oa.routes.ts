...
router.post('/validate-token', async (req: Request, res: Response) => {
  try {
    ...
    const { validateActionToken } = await import('../services/oa/oa-action-token');
...
router.post('/action-by-token', async (req: Request, res: Response) => {
  try {
    ...
    const { validateAndConsumeActionToken } = await import('../services/oa/oa-action-token');
    ...
    const { approveApproval, rejectApproval } = await import('../services/oa/oa.mutation');
    ...
    if (action === 'approve') {
      ...
      res.json({ code: 200, data: { status: result.status }, message: '已通过' });
    } else {
...
// 预解析审批人（发起审批时预览流程用）
router.get('/form-types/:code/preview-approvers', requirePermission('oa:read'), async (req: Request, res: Response) => {
  try {
...
router.post('/form-types/:code/preview-workflow', requirePermission('oa:read'), async (req: Request, res: Response) => {
  try {
    const { code } = req.params;
...
// 获取转交候选人列表
router.get('/transfer-candidates', requirePermission('oa:write'), async (req: Request, res: Response) => {
  try {
    const { appQuery } = await import('../db/appPool');
...
router.get('/erp-reference/:type/resolve', requirePermission(['oa:read', 'oa:write']), resolveErpReference);

// 获取客户营业执照信息（供表单展示已有执照）
router.get('/erp-reference/customers/:id/license-info', requirePermission(['oa:read', 'oa:write']), getCustomerLicense);

// 获取客户欠款总额（供表单展示欠款，用于停用校验）
router.get('/erp-reference/customers/:id/debt', requirePermission(['oa:read', 'oa:write']), getCustomerDebt);
...
// 获取未读消息数量
router.get('/messages/unread-count', requirePermission('oa:read'), getUnreadCount);

// 获取消息列表
router.get('/messages', requirePermission('oa:read'), listMessages);

// 标记消息已读
router.post('/messages/:id/read', requirePermission('oa:write'), readMessage);

// 标记所有消息已读
router.post('/messages/read-all', requirePermission('oa:write'), readAllMessages);
